...
describe("ThemeService", () => {
  ...
  describe("static setTheme", () => {
    it("persists the selected theme via localStorage so it survives a refresh", async () => {
      expect( localStorageMock.getItem( ThemeService.THEME_STORAGE_KEY ) ).toBeUndefined();

      ThemeService.setTheme("dark");
      expect( localStorageMock.setItem ).toHaveBeenCalledWith( ThemeService.THEME_STORAGE_KEY, "dark");
    });
  });
